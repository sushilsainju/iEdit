...
+(void)buttonSelected:(RadioButton*)radioButton{
    ...
    {
        DELEGATE.RecordingSetting=recording_ALAC;
    }
    else
        DELEGATE.RecordingSetting=recording_AAC;
    
    [DEFAULTS setInteger:DELEGATE.RecordingSetting forKey:Key_savedRecorderSetting];
...
-(id)initWithGroupId:(NSString*)groupId index:(NSUInteger)index{
    ...
    if (self) {
        _groupId = groupId;
        _index = index;
        NSLog(@"stored setting %ld",(long)[DELEGATE RecordingSetting]);

            NSLog(@"stored setting %ld",(long)[DELEGATE RecordingSetting]);
            if (_index==[DELEGATE RecordingSetting]) {
                _button.selected=TRUE;// =yes;
            }
