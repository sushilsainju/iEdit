...
- (void) setupDefaultConfigurations{
    ...
    self.sectorsRadius = 47.0;
    self.backgroundColor = [UIColor clearColor];
    self.startAngle = toRadians(270);
    self.minCircleMarkerRadius = 5.0;
    self.maxCircleMarkerRadius = 5.0;
...
- (SASectorDrawingInformation) sectorToDrawInf:(SAMultisectorSector *)sector position:(NSInteger)position{
    ...
    drawInf.circleCenter = CGPointMake(self.bounds.size.width / 2, (self.bounds.size.height-44) /2);
    ...
    if(markersCentresSegmentLength < markersRadiusSumm){
        ...
        drawInf.startMarkerAlpha = 0.7;
    }
    drawInf.endMarkerAlpha = 0.7;
